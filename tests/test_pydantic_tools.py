...
NOTEBOOK = ROOT / "src" / "python_notes" / "tutorials" / "python_tools" / "pydantic_tools.ipynb"


def load_notebook_namespace() -> dict[str, object]:
    """执行 Pydantic notebook 的代码单元。

    Returns:
        包含 notebook 代码单元中定义对象的命名空间。
    """
    ...
    for index, cell in enumerate(data["cells"]):
        ...
        exec(compile(source, f"{NOTEBOOK}:{index}", "exec", dont_inherit=True), namespace)
    return namespace


class PydanticToolsTest(TestCase):
    """检查 Pydantic notebook 示例工具的核心行为。"""
    ...
    def test_summarize_validation_error(self) -> None:
        ...
        self.assertTrue(any(message.startswith("items.0.sku_id:") for message in messages))
        self.assertTrue(any(message.startswith("items.0.quantity:") for message in messages))
        self.assertTrue(any(message.startswith("items.0.unit_price:") for message in messages))
